...
from awsiotclient import mqtt, classic_shadow
...
class ShadowParams():
    def __init__(self, thing_name: str = None, name: str = None, accept_delta: bool = False, publish_full_doc: bool = False) -> None:
...
class Ros2Shadow():
    def __init__(self, conn_params: mqtt.ConnectionParams, shadow_params: ShadowParams) -> None:
        ...
        while topic_class is None:
            try:
                ...
                rospy.logdebug('ROS topic %s is not ready yet. %s raised.', topic_from, e)
            rospy.sleep(1.0)
        ...
        self.shadow_cli = classic_shadow.client(self.mqtt_connection, thing_name=conn_params.thing_name,
            shadow_property=shadow_params.name, delta_func=delta_func)
    ...
    def deny_delta(self, thing_name: str, shadow_name: str, value: dict):
        raise(classic_shadow.ExceptionAwsIotClassicShadowInvalidDelta("this shadow does not accept any delta"))
