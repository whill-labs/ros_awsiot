...
class Jobs2Ros:
    def __init__(
        ...
        self.jobs_cli = jobs.client(
            self.mqtt_connection,
            thing_name=job_params.thing_name,
    ...
    def job_func(self, id, document: dict):
        try:
            if document.get("data") is None:
                raise (
                    jobs.ExceptionAwsIotJobsUserDefinedFailure(
                        "job_id {} is not for me".format(id)
                    )
                )
            ...
            if num_sub < 1:
                raise (
                    jobs.ExceptionAwsIotJobsUserDefinedFailure(
                        "received job_id is {} but there is only {} subscribers".format(
                            id, num_sub
                        )
                    )
                )
...
def main():
    ...
    conn_params.client_id = rospy.get_param(
        "~client_id", default=job_params.thing_name + "-" + str(uuid4())
    )
