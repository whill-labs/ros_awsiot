...
class Ros2Mqtt():
    def __init__(self, topic_from: str, topic_to: str, conn_params: mqtt.ConnectionParams) -> None:
        topic_class = None
        while topic_class is None:
            try:
                ...
                rospy.loginfo('ROS topic %s is not ready yet. %s raised.', topic_from, e)
            rospy.sleep(1.0)
...
def main():
    ...
    conn_params.client_id = rospy.get_param('~client_id', default="ros2mqtt-" + str(uuid4()))
