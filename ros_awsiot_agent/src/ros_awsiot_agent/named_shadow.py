...
import rospy
from rostopic import get_topic_class, get_topic_type, ROSTopicIOException
...
class ShadowParams():
    def __init__(self, thing_name: str = None, name: str = None, enable_downstream: bool = False, enable_upstream: bool = True, publish_full_doc: bool = False) -> None:
...
class Ros2Shadow():
    def __init__(self, conn_params: mqtt.ConnectionParams, shadow_params: ShadowParams) -> None:
        ...
        if shadow_params.enable_upstream:
            while upstream_topic_class is None:
                try:
                    ...
                    rospy.logdebug('ROS topic %s (%s) detected.', upstream_topic, upstream_topic_type)
                except ROSTopicIOException as e:
                    rospy.logdebug('ROS topic %s is not ready yet. %s raised.', upstream_topic, e)
                rospy.sleep(1.0)
        ...
        if shadow_params.enable_downstream:
            while downstream_topic_class is None:
                try:
                    ...
                    rospy.logdebug('ROS topic %s (%s) detected.', downstream_topic, downstream_topic_type)
                except ROSTopicIOException as e:
                    rospy.logdebug('ROS topic %s is not ready yet. %s raised.', downstream_topic, e)
                rospy.sleep(1.0)
        ...
        if downstream_topic_class:
            self.pub = rospy.Publisher(downstream_topic, downstream_topic_class, queue_size=10)
            self.downstream_topic_class = downstream_topic_class
            delta_func = self.accept_delta
        else:
            delta_func = self.deny_delta

        self.shadow_cli = named_shadow.client(self.mqtt_connection, thing_name=shadow_params.thing_name,
            shadow_name=shadow_params.name, delta_func=delta_func, publish_full_doc=shadow_params.publish_full_doc)

        # Subscriber must be initialized after shadow client gets ready
        if upstream_topic_class:
            self.sub = rospy.Subscriber(upstream_topic, upstream_topic_class, callback=self.callback)
    ...
    def deny_delta(self, thing_name: str, shadow_name: str, value: dict):
        raise(named_shadow.ExceptionAwsIotNamedShadowInvalidDelta("this shadow does not accept any delta"))
...
def main():
    ...
    shadow_params.enable_downstream = rospy.get_param('~enable_downstream', default=False)
    ...
    conn_params.client_id = rospy.get_param('~client_id', default=shadow_params.thing_name + "-" + str(uuid4()))
